...
if ($sessionObj->param("role") eq "admin") { 
	...
	if (length($errorMessage) ne 0) {
		...
		updateHost($sessionObj->param("selectedAdmin"), $sessionObj->param("hostName"), $newHostName, $ipAddress);
		$sessionObj->param("hostName",  $newHostName);
		$sessionObj->param("userMessage1", "$newHostName Updated");
	}
	
	$queryString = "hostName=" . URLEncode($newHostName) .
