...
use PerfStatCGI::Utilities1;
use PerfStatCGI::Utilities2;
...
if (!defined($cookies{'perfstatID'}) || !defined($claimedSessionID)) {
	...
	# cookie claimedSessionID is OK and session is loggedIn
		if ($ENV{SCRIPT_NAME} ne "/perfstat/performanceMonitor/content/displayHostGraphs/singleServiceGraphs/drawLineGraph/index.pl"
		&&
		$ENV{SCRIPT_NAME} ne "/perfstat/performanceMonitor/content/displayHostGraphs/singleServiceGraphs/drawGDGraph/index.pl"
		&&
		$ENV{SCRIPT_NAME} ne "/perfstat/performanceMonitor/content/displayHostGraphs/multiServiceGraphs/drawLineGraph/index.pl"
		&&
		$ENV{SCRIPT_NAME} ne "/perfstat/performanceMonitor/content/displayHostGraphs/multiServiceGraphs/drawGDGraph/index.pl"
		&&
		$ENV{SCRIPT_NAME} ne "/perfstat/performanceMonitor/content/displayHGGraphs/singleServiceGraphs/drawGDGraph/index.pl")
		{
			print $request->header();
			CGI::Carp->warningsToBrowser(1);
		}
