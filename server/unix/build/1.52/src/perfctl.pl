# perfctl.pl - PerfStat Client Daemon
...
if ($ENV{'HOST_INFO'} =~ m/y|Y/) {
	...
	$interval_h{'HostInfo'}="$ENV{'INFO_INTERVAL'}";
}
...
sub RunProgram {
	...
	if ($program =~ m/VMSTAT|IOSTAT/) {
		...
		if ($pid == 0) {
			$checkStatus=system("$perfhome/bin/status");
			exit(0);
		}
		...
		if ($pid == 0) {
			$checkAlerts=system("$perfhome/bin/alert");
			exit(0);
		}
		...
		if ($pid == 0) {
			$hostConnectivity=system("$perfhome/bin/conn");
			exit(0);
		}
		...
		if ($pid == 0) {
			$hostInfo=system("$perfhome/bin/info");
			exit(0);
		}
	} else {
		# Run Service program
