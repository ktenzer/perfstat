# conn.pl - Host connectivity check done via ping (icmp)
...
if (-f "$perfhome/tmp/conn.pid") {
	...
	if (kill 0 => $oldPID) {
		...
		if ($pidUser =~ m/$ENV{'USER'}/) {
			warn "INFO: A conn process is already running under PID $oldPID, aborting!\n";
...
sub CREATE_EVENT_LOGS {
        ...
        my $nostatus_filename="$perfhome/var/status/$host/$service.nostatus";
