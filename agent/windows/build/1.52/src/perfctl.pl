...
if ($ENV{'HOST_INFO'} =~ m/y|Y/) {
        ...
        $ENV{'INFO_INTERVAL'}=$ENV{'INFO_INTERVAL'} * 60;
        $intervalBasic_h{'HostInfo'}=$ENV{'INFO_INTERVAL'};
}
    ...
    foreach $i (keys %{$counter_ref0}){
        ...
        if ($instance_ref0->{$p}->{Name} eq "_Total") {
            ...
            if($counter_ref0->{$i}->{CounterNameTitleIndex} == $usr) {
                ...
                # Ensure all metrics have two decimal places and are not negative numbers
                if ($user_time =~ m/^-/) {
                  $user_time=int($user_time);
                } else {
                  $user_time = sprintf "%.2f",$user_time;
                }
            }
            ...
            if($counter_ref0->{$i}->{CounterNameTitleIndex} == $priv) {
                ...
                # Ensure all metrics have two decimal places and are not negative numbers
                if ($priv_time =~ m/^-/) {
                  $priv_time=int($priv_time);
                } else {
                  $priv_time = sprintf "%.2f",$priv_time;
                }
            }
            if($counter_ref0->{$i}->{CounterNameTitleIndex} == $idl) {
                ...
                # Ensure all metrics have two decimal places and are not negative numbers
                if ($idl_time =~ m/^-/) {
                  $idl_time=int($idl_time);
                } else {
                  $idl_time = sprintf "%.2f",$idl_time;
                }
            }
...
sub memory {
    ...
    foreach $i (keys %{$counter_ref0}){
        ...
        if ($instance_ref0->{$p}->{Name} eq "_Total") {
            ...
            if($counter_ref0->{$i}->{CounterNameTitleIndex} == $swap_usage_index) {
                ...
                # Ensure all metrics have two decimal places and are not negative numbers
                if ($swap_used_pct =~ m/^-/) {
                    $swap_used_pct=int($swap_used_pct);
                } else {
                    $swap_used_pct = sprintf "%.2f",$swap_used_pct;
                }
            }
         }
    }
  ...
  # Ensure all metrics have two decimal places and are not negative numbers
  if ($pages_in =~ m/^-/) {
      $pages_in=int($pages_in);
  } else {
      $pages_in = sprintf "%.2f",$pages_in;
  }
  
  # Ensure all metrics have two decimal places and are not negative numbers
  if ($pages_out =~ m/^-/) {
      $pages_out=int($pages_out);
  } else {
      $pages_out = sprintf "%.2f",$pages_out;
  }
  ...
  if ($mem_used_pct =~ m/^-/) {
      ...
      $mem_used_pct = sprintf "%.2f",$mem_used_pct;
  }
  
  my $mem_free_pct=100 - $mem_used_pct;
  # Ensure all metrics have two decimal places and are not negative numbers
  if ($mem_free_pct =~ m/^-/) {
      $mem_free_pct=int($mem_free_pct);
  } else {
      $mem_free_pct = sprintf "%.2f",$mem_free_pct;
  }
  
  my $swap_free_pct=100 - $swap_used_pct;
  # Ensure all metrics have two decimal places and are not negative numbers
  if ($swap_free_pct =~ m/^-/) {
      $swap_free_pct=int($swap_free_pct);
  } else {
      $swap_free_pct = sprintf "%.2f",$swap_free_pct;
  }
          ...
          if ($counter_ref0->{$i}->{CounterNameTitleIndex} == $disk_free_pct_index) {
              ...
              if (defined $Base0 && $Numerator0) {
                  ...
                  if ($disk_free_pct =~ m/^-/) {
                      ...
                      $disk_free_pct = sprintf "%.2f",$disk_free_pct;
                  }
                  ...
                  # Ensure all metrics have two decimal places and are not negative numbers
                  if ($disk_used_pct =~ m/^-/) {
                      $disk_used_pct=int($disk_used_pct);
                  } else {
                      $disk_used_pct = sprintf "%.2f",$disk_used_pct;
                  }
      ...
      foreach my $i (keys %{$counter_ref0}){
          ...
          if ($counter_ref0->{$i}->{CounterNameTitleIndex} == $disk_wait_index) {
              ...
              if (defined $Numerator0 && defined $Denominator0 && defined $TimeBase) {
                  ...
                  # Ensure all metrics have two decimal places and are not negative numbers
                  if ($avg_disk_wait_sec =~ m/^-/) {
                      $avg_disk_wait_sec=int($avg_disk_wait_sec);
                  } else {
                      $avg_disk_wait_sec = sprintf "%.2f",$avg_disk_wait_sec;
                  }
          ...
          if ($counter_ref0->{$i}->{CounterNameTitleIndex} == $disk_busy_index) {
              ...
              if (defined $Numerator0 && defined $Numerator1 && defined $Denominator0 && defined $Denominator1) {
                  ...
                  # Ensure all metrics have two decimal places and are not negative numbers
                  if ($disk_busy_pct =~ m/^-/) {
                      $disk_busy_pct=int($disk_busy_pct);
                  } else {
                      $disk_busy_pct = sprintf "%.2f",$disk_busy_pct;
                  }
          ...
          if ($counter_ref0->{$i}->{CounterNameTitleIndex} == $disk_idle_index) {
              ...
              if (defined $Num0 && defined $Num1 && defined $Den0 && defined $Den1) {
                  ...
                  # Ensure all metrics have two decimal places and are not negative numbers
                  if ($disk_idle_pct =~ m/^-/) {
                      $disk_idle_pct=int($disk_idle_pct);
                  } else {
                      $disk_idle_pct = sprintf "%.2f",$disk_idle_pct;
                  }
          ...
          if (defined $readFlag && defined $writeFlag && defined $waitFlag && defined $busyFlag) {
             ...
             $avg_disk_read_kb = $avg_disk_read_bytes / 1024;
             # Ensure all metrics have two decimal places and are not negative numbers
              if ($avg_disk_read_kb =~ m/^-/) {
                  $avg_disk_read_kb=int($avg_disk_read_kb);
              } else {
                  $avg_disk_read_kb = sprintf "%.2f",$avg_disk_read_kb;
              }

             $avg_disk_write_kb = $avg_disk_write_bytes / 1024;
             # Ensure all metrics have two decimal places and are not negative numbers
             if ($avg_disk_write_kb =~ m/^-/) {
                $avg_disk_write_kb=int($avg_disk_write_kb);
             } else {
                $avg_disk_write_kb = sprintf "%.2f",$avg_disk_write_kb;
             }
        ...
        next if ($instance_ref0->{$p}->{Name} =~ m/MS TCP Loopback interface/);
          ...
          if($counter_ref0->{$i}->{CounterNameTitleIndex} == $bytes_out_index) {
            ...
            if ($bytes_out =~ m/^-/) {
                 ...
                 $bytes_out = sprintf "%.2f",$bytes_out;
            }
...
sub socket {
  ...
  foreach $p (keys %{$instance_ref0}) {
    ...
    foreach $i (keys %{$counter_ref0}){
        ...
        if($counter_ref0->{CounterNameTitleIndex} == $sock_est_index) {
          ...
          # Ensure all metrics have two decimal places and are not negative numbers
          if ($sock_est =~ m/^-/) {
              $sock_est=int($sock_est);
          } else {
              $sock_est = sprintf "%.2f",$sock_est;
          }
        }
  ...
  # Ensure all metrics have two decimal places and are not negative numbers
  if ($sock_active =~ m/^-/) {
      $sock_active=int($sock_active);
  } else {
      $sock_active = sprintf "%.2f",$sock_active;
  }

  # Ensure all metrics have two decimal places and are not negative numbers
  if ($sock_passive =~ m/^-/) {
      $sock_passive=int($sock_passive);
  } else {
      $sock_passive = sprintf "%.2f",$sock_passive;
  }
...
sub SendData {
  ...
  if (defined $socket) {
      ...
      warn "DEBUG: Sending \"$perfOut\" to $ENV{'PERFSERVER'} $ENV{'CLIENTPORT'}\n" if $ENV{'DEBUG'};
