...
#$perfhome=&PATH;
#$perfhome =~ s/\\bin//;
$perfhome="c:/program files/perfstat";
...
@label_col = (
        ...
        $frame1->Label ( -text => "Host Info:", -width => 20 , -relief => 'groove' ),
        $frame1->Label ( -text => "Info Interval:", -width => 20 , -relief => 'groove' ),
...
@data_col = (
        ...
        $frame1->Entry ( -width => 25 , -justify => 'left', -textvariable => \$conf{'HOST_INFO'}),
        $frame1->Entry ( -width => 25 , -justify => 'left', -textvariable => \$conf{'INFO_INTERVAL'}),
