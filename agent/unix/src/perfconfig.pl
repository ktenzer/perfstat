...
sub VerifyUser {
	...
	if (! defined $option1|$option2) {
		&Usage;
	} elsif ($option1 !~ m/client|server|global/) {
		&Usage;
...
sub Client {
	...
	print "\nEnter port running PerfStat server [$ENV{'CLIENTPORT'}]:";
	$ARG=<STDIN>;
	...
	print "\nEnter PerfStat client services [$ENV{'METRICS'}]:";
	$ARG=<STDIN>;
	...
	print "\nEnter PerfStat client Run Interval [$ENV{'RUN_INTERVAL'}]:";
	$ARG=<STDIN>;
	...
	if ($os =~ m/SunOS/) {
                ...
                print "\nEnter path to ps command [$ENV{'PS_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to vmstat command [$ENV{'VMSTAT_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to iostat command [$ENV{'IOSTAT_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to netstat command [$ENV{'NETSTAT_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to df command [$ENV{'DF_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to swap command [$ENV{'SWAP_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to prtconf command [$ENV{'PRTCONF_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to ifconfig command [$ENV{'IFCONFIG_CMD'}]:";
                $ARG=<STDIN>;
                ...
                print "\nEnter path to kstat command [$ENV{'KSTAT_CMD'}]:";
                $ARG=<STDIN>;
...
sub Server {
	...
	if ($ENV{'EMAIL'} =~ m/y|Y/) {
		...
		print "EMAIL_TO=$ENV{'EMAIL_TO'}\n";
		print "EMAIL_FROM=$ENV{'EMAIL_FROM'}\n";
		print "EMAIL_SUBJECT=$ENV{'EMAIL_SUBJECT'}\n"
	}
...
sub EmailConfiguration {
	...
	# Configure PerfStat TO email address
	print "Enter email alert TO address [$ENV{'EMAIL_TO'}]:";
	$ARG=<STDIN>;
	...
	# Configure PerfStat email subject
	print "Enter email alert SUBJECT [$ENV{'EMAIL_SUBJECT'}]:";
	$ARG=<STDIN>;

	# Ensure argument is not blank
	if ($ARG !~ m/^\s+/) {
		chomp $ARG;
		$ENV{'EMAIL_SUBJECT'}=$ARG;
	}
}
