...
use warnings;
use lib "/perfstat/dev/1.52/server/lib";
...
sub RRD {
	...
	warn "DEBUG: RRD File $RRD\n" if ($ENV{'DEBUG'});
		...
		my $rrdAPI=`$perfhome/bin/rrdAPI.pl new,$host,$service,$rrdStep,@ds,$RRA`;
		...
		my $rrdAPI=`$perfhome/bin/rrdAPI.pl update,$host,$service,$dataUpdate`;
