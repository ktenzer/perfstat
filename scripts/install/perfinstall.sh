...
SetVars () {
	...
	if [ "$SERVER" = "y" -o "$SERVER" = "Y" ]; then
		...
		if [ ! -f "/usr/lib/lib/libgd.so.2 -o /usr/lib/libgd.so.2" ];then
			$COPY_CMD $PERFHOME/lib/GD/libgd.so.2 /usr/lib
		fi
		if [ ! -f "/usr/lib/lib/libpng12.so.0 -o /usr/lib/libpng12.so.0" ];then
			$COPY_CMD $PERFHOME/lib/GD/libpng12.so.0 /usr/lib
		fi
			
		$CAT_CMD $PERFHOME/etc/perf-conf |$SED_CMD "s/PERFHOME=.*/PERFHOME=$PERFHOME_SED/" |$SED_CMD "s/SERVERIP=.*/SERVERIP=$IP_SED/" |$SED_CMD "s/PERFSERVER=.*/PERFSERVER=$IP_SED/" |$SED_CMD "s/USER=.*/USER=$USER/" |$SED_CMD "s/GROUP=.*/GROUP=$GROUP/" > $PERFHOME/etc/perf-conf.save
...
ConfigureApp () {
	...
	if [ "$SERVER" = "y" -o "$SERVER" = "Y" ]; then
		$PERFHOME/bin/tools/perfconfig.pl -global -root
		$PERFHOME/bin/tools/perfconfig.pl -server -root
		$PERFHOME/bin/tools/perfconfig.pl -client -root
	else
		$PERFHOME/bin/tools/perfconfig.pl -global -root
		$PERFHOME/bin/tools/perfconfig.pl -client -root
	fi
