...
use vars qw($action $queryString $adminName $userName $updateNav $displayMode $contentType
			...
			$numColumns $graphSize $customGraphSize $customFontSize $useShortDomainNames
			);
$request = new CGI;
...
elsif ($action eq "moveContentDown")
{
	require("act_moveContentDown.pl");
	metaRedirect(0, "index.pl?$queryString");
}
elsif ($action eq "moveContentUp")
{
	require("act_moveContentUp.pl");
	metaRedirect(0, "index.pl?$queryString");
}
...
elsif ($action eq "updateReportLayout")
{
	require("act_updateReportLayout.pl");
	metaRedirect(0, "index.pl?$queryString");
}
