...
sub checkItemName {
	...
	if(-f "$perfhome/var/db/users/$adminName/metricTemplates/$itemName.ser") {return("Template Name is already taken");}
}
...
sub checkEditTemplateName {
	...
	if ( $templateName ne $newTemplateName) {
		if(-f "$perfhome/var/db/users/$adminName/metricTemplates/$newTemplateName.ser") {return("Template Name is already taken");}
	}
}
