...
sub insertTemplate {
	...
	# Create AlertTemplate
	my $alertTemplate = AlertTemplate->new(	name => $addName,
													description => $addDescription);
	
	# Serialize host data to disk (host.ser)
	$alertTemplate->lock_store("$perfhome/var/db/users/$adminName/alertTemplates/$addName.ser") or die "ERROR: Can't store $perfhome/var/db/users/$adminName/alertTemplates/$addName.ser\n";
}
