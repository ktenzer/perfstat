...
if ($sessionObj->param("role") eq "admin") {
	$newosName = $request->param('newosName');
	checkOSName($newosName);
	...
	if (length($errorMessage) eq 0) {
		...
		$errorMessage = checkIPAddress($newipAddress);
	}
	if (length($errorMessage) != 0) {
		$sessionObj->param("userMessage1", $errorMessage);
		$queryString = "newHostName=" . URLEncode($newHostName) .
						"&newipAddress=" . URLEncode($newipAddress) .
						"&newosName=$newosName";
	} else {
		insertHost($sessionObj->param("selectedAdmin"), $newHostName, $newipAddress, $newosName);
		$queryString = "";
	}
...
sub insertHost {
	...
	# Create host services directory
