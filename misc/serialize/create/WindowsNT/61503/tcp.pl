...
$obj = Metric->new(	rrdIndex => 1,
					metricName => "recieved_kb",
					friendlyName => "TCP: kb received",
					hasEvents => 1,
					warnThreshold => 10000,
					critThreshold => 25000,
					thresholdUnit => "kilobytes",
...
$obj = Graph->new(	name   => "traffic",
					...
					legend  => "",
					optionsArray => [],
					defArray => [q{DEF:bytes_in=$RRD:tcp_recieved_kb:AVERAGE},q{DEF:bytes_out=$RRD:tcp_sent_kb:AVERAGE}],
					cdefArray => [],
