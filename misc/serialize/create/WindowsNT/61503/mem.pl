...
$obj = Metric->new(	rrdIndex => 0,
					metricName => "avail_mb",
					friendlyName => "Memory Available (MB)",
					hasEvents => 0,
					warnThreshold => 0,
					critThreshold => 0,
					thresholdUnit => "MB",
...
$obj = Metric->new(	rrdIndex => 1,
					metricName => "commit_mb",
					friendlyName => "Memory Commit (MB)",
					hasEvents => 0,
					warnThreshold => 0,
					critThreshold => 0,
					thresholdUnit => "?",
...
$obj = Metric->new(	rrdIndex => 3,
					metricName => "pages_out",
					friendlyName => "Mem Pages Out",
					hasEvents => 0,
					warnThreshold => 0,
					critThreshold => 0,
					thresholdUnit => "?",
...
$obj = Graph->new(	name => "paging",
					...
					defArray => [q{DEF:page_in=$RRD:mem_pages_in:AVERAGE},q{DEF:page_out=$RRD:mem_pages_out:AVERAGE}],
					cdefArray => [],
