...
$obj = Metric->new(	rrdIndex => 0,
					metricName => "uptimeHour",
					friendlyName => "System Uptime",
					status => "nostatus",
					rrdDST => GAUGE,
					rrdHeartbeat => 600,
					rrdMin => 0,
...
$obj2 = GraphMetric->new(       name => "uptimeHour",
                                                color           => "#800000",
                                                lineType        => "LINE2",
                                                gprintArray     => [qw{AVERAGE LAST}],
                                                cDefinition     => "",
                                                );
$obj->addGraphMetric("uptimeHour", $obj2);

$service->addGraph("uptime", $obj);
