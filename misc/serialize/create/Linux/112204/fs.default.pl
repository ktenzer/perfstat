...
$obj = Metric->new(	rrdIndex => 0,
					metricName => "inodeUsedPct",
					friendlyName => "Inode % Utilization",
					status => "nostatus",
					rrdDST => GAUGE,
					rrdHeartbeat => 600,
					rrdMin => 0,
...
$obj = Metric->new(	rrdIndex => 1,
					metricName => "fsUsedPct",
					friendlyName => "File System % Utilization",
					status => "nostatus",
					rrdDST => GAUGE,
					rrdHeartbeat => 600,
					rrdMin => 0,
...
$obj2 = GraphMetric->new(       name => "inodeUsedPct",
                                                color           => "#00FF00",
                                                lineType        => "LINE2",
                                                gprintArray     => [qw{AVERAGE LAST}],
                                                cDefinition     => "",
                                                );
$obj->addGraphMetric("inodeUsedPct", $obj2);

$obj2 = GraphMetric->new(       name => "fsUsedPct",
                                                color           => "#0000FF",
                                                lineType        => "LINE2",
                                                gprintArray     => [qw{AVERAGE LAST}],
                                                cDefinition     => "",
                                                );
$obj->addGraphMetric("fsUsedPct", $obj2);
...
#Store the service
$service->store("$perfhome/etc/configs/Linux/$service->{serviceName}.default.ser") or die("can't store $service->{serviceName}.default.ser?\n");
