...
$obj = Metric->new(	rrdIndex => 0,
					metricName => "new_sec",
					friendlyName => "New Processes/Second",
					hasEvents => 1,
					warnThreshold => 25,
					critThreshold => 50,
					thresholdUnit => "new processes/second",
					);
...
$obj = Graph->new(	name   => "Number Processes",
					...
					legend  => "",
					optionsArray => [],
					defArray => [q{DEF:total=$RRD:total:AVERAGE}],
					cdefArray => [],
					lineArray => [qq{LINE2:total#0000ff:Total}],
					text    => "",
					);
