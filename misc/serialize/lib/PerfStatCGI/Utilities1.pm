...
sub checkAndSetHostGroupID {
	...
	if (!$allowUndefinedRequestParam && !defined($request->param("hostGroupID"))) {
		...
		$hostGroupID = defined($request->param('hostGroupID')) ? $request->param('hostGroupID') : "All Hosts";
	}

	if (!defined($sessionObj->param("hostGroupID"))) {$sessionObj->param("hostGroupID", "allHosts");}

	if ($sessionObj->param("hostGroupID") ne $hostGroupID) {
		if ($hostGroupID =~ m/All Hosts/) {
			...
			my $hostGroupList = $user2HostGroup->{$sessionObj->param("selectedAdmin")}->{$sessionObj->param("selectedUser")}->{$sessionObj->param("selectedUser")};
	...
	if ($hostGroupID =~ /All Hosts/) {
		$hostGroupName = "All Hosts";
	} else {
		my $hgOwner = $sessionObj->param("selectedUser");
		my $hostGroupObject = lock_retrieve("$perfhome/var/db/users/$hgOwner/hostGroups/$hostGroupID.ser") or die("Error: can't retrieve $perfhome/var/db/users/$hgOwner/hostGroups/$hostGroupID.ser\n");
		$hostGroupName = $hostGroupObject->getName();
	}
}
...
sub deleteHost {
	...
	foreach my $userName (keys(%$userNameList)) {
		...
		foreach my $hostGroupName (keys(%$hostGroupList)) {
			updateHostGroup($userName, $hostGroupName, $hostName);
		}
	}
